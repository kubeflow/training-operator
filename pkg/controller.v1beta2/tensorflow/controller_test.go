...
func TestNormalPath(t *testing.T) {
	...
	for name, tc := range testCases {
		...
		for _, controllerRef := range fakePodControl.ControllerRefs {
			if got, want := controllerRef.APIVersion, tfv1beta2.SchemeGroupVersion.String(); got != want {
				t.Errorf("controllerRef.APIVersion = %q, want %q", got, want)
			}
			if got, want := controllerRef.Kind, tfv1beta2.Kind; got != want {
		...
		if actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypeWorker)] != nil {
			if actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypeWorker)].Active != tc.expectedActiveWorkerPods {
				t.Errorf("%s: unexpected number of active pods.  Expected %d, saw %d\n",
					name, tc.expectedActiveWorkerPods,
					actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypeWorker)].Active)
			}
			if actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypeWorker)].Succeeded != tc.expectedSucceededWorkerPods {
				t.Errorf("%s: unexpected number of succeeded pods.  Expected %d, saw %d\n",
					name, tc.expectedSucceededWorkerPods,
					actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypeWorker)].Succeeded)
			}
			if actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypeWorker)].Failed != tc.expectedFailedWorkerPods {
				t.Errorf("%s: unexpected number of failed pods.  Expected %d, saw %d\n",
					name, tc.expectedFailedWorkerPods,
					actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypeWorker)].Failed)
			}
		}
		// Validate PS status.
		if actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypePS)] != nil {
			if actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypePS)].Active != tc.expectedActivePSPods {
				t.Errorf("%s: unexpected number of active pods.  Expected %d, saw %d\n",
					name, tc.expectedActivePSPods,
					actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypePS)].Active)
			}
			if actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypePS)].Succeeded != tc.expectedSucceededPSPods {
				t.Errorf("%s: unexpected number of succeeded pods.  Expected %d, saw %d\n",
					name, tc.expectedSucceededPSPods,
					actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypePS)].Succeeded)
			}
			if actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypePS)].Failed != tc.expectedFailedPSPods {
				t.Errorf("%s: unexpected number of failed pods.  Expected %d, saw %d\n",
					name, tc.expectedFailedPSPods,
					actual.Status.ReplicaStatuses[common.ReplicaType(tfv1beta2.TFReplicaTypePS)].Failed)
			}
		}
		// Validate StartTime.
