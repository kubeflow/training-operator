...
func TestCopyLabelsAndAnnotation(t *testing.T) {
	...
	tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypeWorker].Template.Labels = labels
	tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypeWorker].Template.Annotations = annotations
	unstructured, err := testutil.ConvertTFJobToUnstructured(tfJob)
	if err != nil {
		t.Errorf("Failed to convert the TFJob to Unstructured: %v", err)
	}
