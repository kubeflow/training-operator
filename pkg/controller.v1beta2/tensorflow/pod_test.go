...
func TestClusterSpec(t *testing.T) {
	...
	for _, c := range testCase {
		...
		demoTemplateSpec := c.tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypeWorker].Template
...
func TestRestartPolicy(t *testing.T) {
	type tc struct {
		...
		expectedType          tfv1beta2.TFReplicaType
	}
	testCase := []tc{
		func() tc {
			tfJob := testutil.NewTFJob(1, 0)
			specRestartPolicy := common.RestartPolicyExitCode
			tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypeWorker].RestartPolicy = specRestartPolicy
			return tc{
				tfJob: tfJob,
				expectedRestartPolicy: v1.RestartPolicyNever,
				expectedType:          tfv1beta2.TFReplicaTypeWorker,
			}
		}(),
		func() tc {
			tfJob := testutil.NewTFJob(1, 0)
...
func TestExitCode(t *testing.T) {
	...
	tfJob := testutil.NewTFJob(1, 0)
	tfJob.Spec.TFReplicaSpecs[tfv1beta2.TFReplicaTypeWorker].RestartPolicy = common.RestartPolicyExitCode
	unstructured, err := testutil.ConvertTFJobToUnstructured(tfJob)
	if err != nil {
		t.Errorf("Failed to convert the TFJob to Unstructured: %v", err)
	}
