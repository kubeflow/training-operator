...
func validateXGBReplicaSpecs(rSpecs map[trainingoperator.ReplicaType]*trainingoperator.ReplicaSpec) field.ErrorList {
	...
	for rType, rSpec := range rSpecs {
		...
		validReplicaTypes := []string{
			string(trainingoperator.XGBoostJobReplicaTypeMaster),
			string(trainingoperator.XGBoostJobReplicaTypeWorker),
		}
		if !slices.Contains(validReplicaTypes, string(rType)) {
