...
func (tc *TFJobController) syncTFJob(key string) (bool, error) {
	...
	if tc.config.enableGangScheduling {
		...
		if err != nil {
			log.Warnf("Sync pdb error: %v", err)
		}
	}
...
func (tc *TFJobController) syncPdb(tfjob *tfv1alpha2.TFJob) (*v1beta1.PodDisruptionBudget, error) {
	...
	pdb, err := tc.kubeClientSet.PolicyV1beta1().PodDisruptionBudgets(tfjob.Namespace).Get(pdbName, metav1.GetOptions{})
	if err == nil || !k8serrors.IsNotFound(err) {
		log.Warnf("PDB: %v already exists or %v", pdbName, err)
		if err == nil {
