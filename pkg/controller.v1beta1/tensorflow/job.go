...
func (tc *TFController) addTFJob(obj interface{}) {
	...
	if err != nil {
		...
		if err == errFailedMarshal {
			...
			client, err := k8sutil.NewCRDRestClient(&tfv1beta1.SchemeGroupVersion)
			...
			if err == nil {
				...
				err = client.Update(un, tfv1beta1.Plural)
				if err != nil {
...
func (tc *TFController) deletePodsAndServices(tfJob *tfv1beta1.TFJob, pods []*v1.Pod) error {
	if len(pods) == 0 {
		return nil
	}
...
func (tc *TFController) cleanupTFJob(tfJob *tfv1beta1.TFJob) error {
...
func (tc *TFController) deleteTFJob(tfJob *tfv1beta1.TFJob) error {
	return tc.tfJobClientSet.KubeflowV1beta1().TFJobs(tfJob.Namespace).Delete(tfJob.Name, &metav1.DeleteOptions{})
}
