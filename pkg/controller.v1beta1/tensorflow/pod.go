	...
	for index, podSlice := range podSlices {
		...
		if len(podSlice) > 1 {
			...
			for _, status := range pod.Status.ContainerStatuses {
				state := status.State
				if status.Name == tfv1beta1.DefaultContainerName && state.Terminated != nil {
			...
			if rtype == tfv1beta1.TFReplicaTypeWorker && index == 0 && exitCode == 0 {
...
func setClusterSpec(podTemplateSpec *v1.PodTemplateSpec, tfjob *tfv1beta1.TFJob, rt, index string) error {
