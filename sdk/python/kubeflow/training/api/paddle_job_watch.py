...
def watch(name=None, namespace=None, timeout_seconds=600):
    ...
    for event in stream:
        paddlejob = event["object"]
        paddlejob_name = paddlejob["metadata"]["name"]
        if name and name != paddlejob_name:
            continue
        else:
            status = ""
            update_time = ""
            ...
            tbl(paddlejob_name, status, update_time)

            if name == paddlejob_name:
                if status in [
