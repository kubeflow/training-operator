...
go run ${OPENAPI_PKG}/cmd/openapi-gen \
  ...
  --output-pkg "${TRAINING_OPERATOR_PKG}/pkg/apis/trainer.kubeflow.org/v2alpha1" \
  --output-dir "${TRAINING_OPERATOR_ROOT}/pkg/apis/trainer.kubeflow.org/v2alpha1" \
  --output-file "zz_generated.openapi.go" \
  --report-filename "${TRAINING_OPERATOR_ROOT}/hack/violation_exception_v2alpha1.list" \
  "${TRAINING_OPERATOR_ROOT}/pkg/apis/trainer.kubeflow.org/v2alpha1"
