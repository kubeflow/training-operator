  ...
  def __init__(self, batch_env):
    ...
    with tf.variable_scope('env_temporary'):
      self._observ = tf.Variable(
          ...
          name='observ', trainable=False, collections=[tf.GraphKeys.LOCAL_VARIABLES])
      self._action = tf.Variable(
          tf.zeros((len(self._batch_env),) + action_shape, action_dtype),
          name='action', trainable=False, collections=[tf.GraphKeys.LOCAL_VARIABLES])
      self._reward = tf.Variable(
          tf.zeros((len(self._batch_env),), tf.float32),
          name='reward', trainable=False, collections=[tf.GraphKeys.LOCAL_VARIABLES])
      self._done = tf.Variable(
          tf.cast(tf.ones((len(self._batch_env),)), tf.bool),
          name='done', trainable=False, collections=[tf.GraphKeys.LOCAL_VARIABLES])
