...
class StreamingNormalize(object):
  ...
  def __init__(
    ...
    with tf.name_scope(name):
      self._count = tf.Variable(0, False, collections=[
          tf.GraphKeys.LOCAL_VARIABLES])
      self._mean = tf.Variable(tf.zeros_like(template), False, collections=[
          tf.GraphKeys.LOCAL_VARIABLES])
      self._var_sum = tf.Variable(tf.zeros_like(template), False, collections=[
          tf.GraphKeys.LOCAL_VARIABLES])
